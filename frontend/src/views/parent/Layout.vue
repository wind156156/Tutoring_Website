...
const menuOptions = [
  { label: '学生管理', key: '/parent/students', icon: '👨‍🎓' },
  { label: '学生作业', key: '/parent/student-assignments', icon: '📝' },
  { label: '平台老师', key: '/parent/teachers', icon: '👩‍🏫' },
  { label: '收藏老师', key: '/parent/favorites', icon: '⭐' },
  { label: '绑定申请', key: '/parent/bindings', icon: '🔗' },
]

const roleMenuOptions = [
  { label: store.user?.role === 'parent' ? '家长中心' : store.user?.role, key: 'role', disabled: true },
  { type: 'divider' as const },
  { label: '退出登录', key: 'logout' },
]

function isActive(key: string): boolean {
  if (key === '/parent/students') return /^\/parent\/students(\/|$)/.test(route.path) && !/^\/parent\/students\/\d+\/assignments/.test(route.path)
...
<template>
  <n-layout has-sider style="height: 100vh">
    <n-layout-sider :width="220" bordered collapse-mode="width" :collapsed-width="64" style="background: linear-gradient(180deg, #4f7cff 0%, #3a5fbf 100%);">
      <div class="sider-logo">
        <span style="font-size: 13px; font-weight: 500; opacity: 0.8; display: block; text-align: center; letter-spacing: 1px;">家教平台</span>
        <span style="font-size: 15px; font-weight: 700; display: block; text-align: center;">家长中心</span>
      </div>
      <div style="padding: 8px 0;">
        <div v-for="item in menuOptions" :key="item.key"
          :class="['menu-item', isActive(item.key) ? 'active' : '']"
    ...
    <n-layout>
      <n-layout-header style="padding: 0 24px; height: 60px; display: flex; align-items: center; justify-content: flex-end; border-bottom: 1px solid var(--color-border); background: var(--color-surface);">
        <n-dropdown :options="roleMenuOptions" @select="handleDropdownSelect" placement="bottom-end">
          <div class="user-info">
            <n-avatar :size="32" style="background: linear-gradient(135deg, #4f7cff, #7c3aed); border-radius: 8px;">
