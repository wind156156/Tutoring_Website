...
const menuOptions = [
  { label: '我的资料', key: '/teacher/profile', icon: '👤' },
  { label: '绑定申请', key: '/teacher/bindings', icon: '🔗' },
  { label: '我的学生', key: '/teacher/students', icon: '👨‍🎓' },
  { label: '作业管理', key: '/teacher/assignments', icon: '📝' },
  { label: '平台老师', key: '/teacher/platform-teachers', icon: '👥' },
]

const roleMenuOptions = [
  { label: '老师中心', key: 'role', disabled: true },
...
<template>
  <n-layout has-sider style="height: 100vh">
    <n-layout-sider :width="220" bordered collapse-mode="width" :collapsed-width="64" style="background: linear-gradient(180deg, #86efac 0%, #4ade80 100%);">
      <div class="sider-logo">
        <span style="font-size: 13px; font-weight: 500; opacity: 0.8; display: block; text-align: center; letter-spacing: 1px;">家教平台</span>
        <span style="font-size: 15px; font-weight: 700; display: block; text-align: center;">老师中心</span>
      </div>
    ...
    <n-layout>
      <n-layout-header style="padding: 0 24px; height: 60px; display: flex; align-items: center; justify-content: flex-end; border-bottom: 1px solid var(--color-border); background: var(--color-surface);">
        <n-dropdown :options="roleMenuOptions" @select="handleDropdownSelect" placement="bottom-end">
          <div class="user-info">
            <n-avatar :size="32" style="background: linear-gradient(135deg, #86efac, #4ade80); border-radius: 8px;">
