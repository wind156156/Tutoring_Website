...
<template>
  <div v-if="!loading">
    <n-button text type="primary" style="margin-bottom: 16px; font-weight: 500;" @click="router.back()">← 返回列表</n-button>
    <n-result
      v-if="!teacher"
      status="404"
      title="老师不存在"
    ...
    >
      <template #footer>
        <n-button type="primary" @click="router.push('/student/teachers')">返回列表</n-button>
      </template>
    </n-result>
    <div v-else class="fade-in">
      <div class="detail-header">
        ...
        <div class="detail-title-row">
          ...
          <div style="margin-top: 10px;">
            <n-tag v-for="s in (teacher.subject_tags || [])" :key="s" type="info" style="margin-right: 4px;">{{ s }}</n-tag>
      ...
      <div class="section-card" style="margin-top: 20px; display: flex; align-items: center; gap: 16px;">
        <div>
