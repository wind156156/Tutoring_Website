...
const menuOptions = [
  ...
  { label: '老师审核', key: '/admin/teachers', icon: '✅' },
  { label: '公告管理', key: '/admin/announcements', icon: '📢' },
  { label: '数据统计', key: '/admin/stats', icon: '📊' },
]

const roleMenuOptions = [
  { label: '管理后台', key: 'role', disabled: true },
...
<template>
  <n-layout has-sider style="height: 100vh">
    <n-layout-sider :width="220" bordered collapse-mode="width" :collapsed-width="64" style="background: linear-gradient(180deg, #1e293b 0%, #0f172a 100%);">
      <div class="sider-logo">
        <span style="font-size: 13px; font-weight: 500; opacity: 0.7; display: block; text-align: center; letter-spacing: 1px;">管理后台</span>
        <span style="font-size: 15px; font-weight: 700; display: block; text-align: center;">家教平台</span>
      </div>
    ...
    <n-layout>
      <n-layout-header style="padding: 0 24px; height: 60px; display: flex; align-items: center; justify-content: flex-end; border-bottom: 1px solid var(--color-border); background: var(--color-surface);">
        <n-dropdown :options="roleMenuOptions" @select="handleDropdownSelect" placement="bottom-end">
          <div class="user-info">
            <n-avatar :size="32" style="background: linear-gradient(135deg, var(--color-primary), #7c3aed); border-radius: 8px;">
